...
- sortir qu'est-ce que le langage sql et pourquoi que c'est important de le connaître
...
Avant de s'attaquer à la récupération de données, faisons un retour sur les notions SQL.
...
Dans cet exemple, nous souhaitons consulter les données dans les colonnes nom et prenom.  Vous pouvez modifier, __uniquement pour le résultat de la requête__, le nom d’une colonne avec l’instruction __AS__.
...
Les opérateurs arithmétiques exécutent des opérations mathématiques sur deux expressions d’un ou plusieurs types de données. Ils sont exécutés à partir de la catégorie de type données numériques.
